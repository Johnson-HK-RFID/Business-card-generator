...
印刷用途請优先分别下载「正面 SVG」和「背面 SVG」：Logo、QR Code、文字、线条和装饰都以矢量元素输出，尺寸为 90 × 43.5 mm。两个下载按钮不受当前预览面影响。SVG 文字保持可编辑；交给印刷商时应在 Illustrator、Affinity Designer 或 Inkscape 中将文字转为轮廓，以完全避免字体替换。原有 PNG 与双面 PDF 维持不变，方便预览和一般打印。
